import {app, BrowserWindow, globalShortcut, ipcMain, dialog} from 'electron';  //globalShortcut must be defined with app or it does not work
...
log.warn('main.mjs:  Starting application\n\n' +
         '*********************************\n' + 
         '* dayjob started...             *\n' + 
         '*********************************\n');

log.warn('main.mjs:  \'app.getAppPath\' path is reported as: ' + app.getAppPath());
...
import * as spotifyServer from './spotify-server.mjs';
import * as prefsLocal from './prefs.mjs';
import fs from 'fs';
import path from 'path'; // For constructing URLs
...
process.on('unhandledRejection', err => {
  log.warn('main.mjs:  [ERROR]  Unhandled promise rejection.  This should be logged as a bug on the dayjob GitHub page.  Error:  \n\n' + String(err.message) + '\n' + 
  'Stack:   ' + err.stack);
...
let notifications = JSON.parse(fs.readFileSync(path.join(app.getAppPath(),'/src/notification.json')));
log.warn('main.mjs:  Loaded notification/error handling database') 
...
const mb = menubar({preloadWindow: true,
                    ...
                    icon: path.join(app.getAppPath(), '/assets/IconTemplate.png'),
...
function openPrefsWindow() {
  log.warn('main.mjs:  Opening preferences window... ');
...
function connectApi(){
  log.warn('main.mjs:  Attempting to connect to the Spotify API...');
...
function authoriseDayjob(){
  if (spotifyServer.getWebServer() == undefined){
    ...
    log.warn('main.mjs:  The web server is already running.');
  }
  ...
  (async () => {
    try {
      ...
      // Launch the auth URL
      log.warn('main.mjs:  Launching the Spotify authorisation URL...');
      ...
      log.warn('main.mjs:  Unable to launch the Spotify authorisation URL...');
      console.error(error);
    }
  })();
}
...
spotifyServer.getAuthEvents().on('auth_code_grant_error', err => { 
  console.log('main.mjs:  Auth event \'auth_code_grant_error\' has been raised by spotify-server.mjs, reporting the error.... ' +  err);
...
spotifyServer.getAuthEvents().on('auth_code_grant_success', result => {
  spotifyServer.stopWebServer();
  console.log('main.mjs:  Auth event \'auth_code_grant_success\' has been raised by spotify-server.mjs, now connecting API... ' +  result);
  connectApi();
})
...
spotifyServer.getAuthEvents().on('ready', result => {   
  ...
  log.warn('main.mjs:  Auth event \'ready\' has been raised by spotify-server.mjs.');
})  

spotifyServer.getAuthEvents().on('server_stopped', result => {  
  // Information only
  log.warn('main.mjs:  Auth event \'server_stopped\' has been raised by spotify-server.mjs.');
})  

spotifyServer.getAuthEvents().on('webserver_port_in_use', result => {  
  log.warn('main.mjs:  [ERROR] Auth event \'webserver_port_in_use\' has been raised by spotify-server.mjs.  Port 8888 is in use but required by dayjob to authorise with Spotify.  Ensure port is free and start dayjob again.  Error: ' + String(result.message));
...
spotifyServer.getAuthEvents().on('webserver_general_error', result => {  
    log.warn('main.mjs:  [ERROR] Auth event \'webserver_general_error\' has been raised by spotify-server.mjs.  Error: ' + String(result.message));
...
app.on('ready', () => {
  log.warn('main.mjs:  app.on ready event occurred...');
});

// When menubar is loaded open the notification window to initialise it
mb.on('ready', () => {
  log.warn('main.mjs:  mb.on ready event occurred...');
  ...
  mb.tray.on('click', () => {
    log.warn('main.mjs:  mb.tray.on click event occurred...');
  });

  //Context menu open
  mb.tray.on('right-click', () => {
    log.warn('main.mjs:  mb.tray.on right-click event occurred...');
...
mb.on('after-create-window', (ready) => {
  log.warn('main.mjs:  mb.on after-create-window event occurred...');

  // Menubar requires the window to be fully loaded before we can send data to it
  mb.window.webContents.on('did-finish-load', () => {
    log.warn('main.mjs:  mb.window.webContents.on did-finish-load event occurred...');
...
function registerKeyboardShortcuts(){
  ...
  //TODO: Should alert the user if a keyboard shortcut fails
  if (!ctrlAlt1) {log.warn('main.mjs:  registration failed of: ctrlAlt1 (Control+Alt+1)')};
  if (!ctrlAlt2) {log.warn('main.mjs:  registration failed of: ctrlAlt2 (Control+Alt+2)')};
  if (!ctrlAlt3) {log.warn('main.mjs:  registration failed of: ctrlAlt3 (Control+Alt+3)')};
  if (!ctrlAlt4) {log.warn('main.mjs:  registration failed of: ctrlAlt4 (Control+Alt+4)')};
  if (!ctrlAlt5) {log.warn('main.mjs:  registration failed of: ctrlAlt5 (Control+Alt+5)')};
  if (!ctrlAlt6) {log.warn('main.mjs:  registration failed of: ctrlAlt6 (Control+Alt+6)')};
  if (!ctrlAlt7) {log.warn('main.mjs:  registration failed of: ctrlAlt7 (Control+Alt+7)')};
  if (!ctrlAlt8) {log.warn('main.mjs:  registration failed of: ctrlAlt8 (Control+Alt+8)')};
  if (!ctrlAlt9) {log.warn('main.mjs:  registration failed of: ctrlAlt9 (Control+Alt+9)')};
  if (!ctrlAlt0) {log.warn('main.mjs:  registration failed of: ctrlAlt0 (Control+Alt+0)')};
  if (!ctrlAltMinus) {log.warn('main.mjs:  registration failed of: ctrlAltMinus (Control+Alt+-)')};
  if (!ctrlAltPlus) {log.warn('main.mjs:  registration failed of: ctrlAltPlus (Control+Alt+=)')};
  // Warn if registration of CRTL + ALT + SHIFT shortcuts fail
  //TODO: Should alert  the user if a keuyboard shortcut fails
  if (!ctrlAltShf1) {log.warn('main.mjs:  registration failed of: ctrlAlt1 (Control+Alt+Shift+1)')};
  if (!ctrlAltShf2) {log.warn('main.mjs:  registration failed of: ctrlAlt2 (Control+Alt+Shift+2)')};
  if (!ctrlAltShf3) {log.warn('main.mjs:  registration failed of: ctrlAlt3 (Control+Alt+Shift+3)')};
  if (!ctrlAltShf4) {log.warn('main.mjs:  registration failed of: ctrlAlt4 (Control+Alt+Shift+4)')};
  if (!ctrlAltShf5) {log.warn('main.mjs:  registration failed of: ctrlAlt5 (Control+Alt+Shift+5)')};
  if (!ctrlAltShf6) {log.warn('main.mjs:  registration failed of: ctrlAlt6 (Control+Alt+Shift+6)')};
  if (!ctrlAltShf7) {log.warn('main.mjs:  registration failed of: ctrlAlt7 (Control+Alt+Shift+7)')};
  if (!ctrlAltShf8) {log.warn('main.mjs:  registration failed of: ctrlAlt8 (Control+Alt+Shift+8)')};
  if (!ctrlAltShf9) {log.warn('main.mjs:  registration failed of: ctrlAlt9 (Control+Alt+Shift+9)')};
  if (!ctrlAltShf0) {log.warn('main.mjs:  registration failed of: ctrlAlt0 (Control+Alt+Shift+0)')};
  if (!ctrlAltShfMinus) {log.warn('main.mjs:  registration failed of: ctrlAltMinus (Control+Alt+Shift+-)')};
  if (!ctrlAltShfPlus) {log.warn('main.mjs:  registration failed of: ctrlAltPlus (Control+Alt+Shift+=)')};
...
function keyPressed(key){
  log.warn('main.mjs:  Keyboard shortcut pressed: ' + JSON.stringify(key.modifiers) + ' AND ' + key.key);
 ...
 */
  ...
  if (key.modifiers.includes('Control') && key.modifiers.includes('Alt') && key.key == '-'){
    let skip = 0; // Determine if track should be skipped as well as moved
    ...
    log.warn('main.mjs:  Remove track keyboard shortcut pressed...');
 ...
 */
  ...
  if (key.modifiers.includes('Control') && key.modifiers.includes('Alt') && key.key == '='){
    log.warn('main.mjs:  Add/move track to specified playlist (+) shortcut pressed... THIS FEATURE ISN\'T SUPPORTED YET SORRY!');
 ...
 */
  ...
  if (key.modifiers.includes('Control') && key.modifiers.includes('Alt') && ['1','2','3','4','5','6','7','8','9','0'].includes(key.key)){
    ...
    return Promise.resolve().then(result => {
      ...
      log.warn('main.mjs:  Add/move track to playlist in slot shortcut pressed:  Slot: ' + key.key + ' Move: ' + move);
...
function logError(err){
  try {
    log.warn('main.mjs:  [ERROR] has occurred:  ' + err + '\n' + 
            'Stack:   ' + err.stack)
    
    // Check if the error was caused by an external module and log it     
    if (err.hasOwnProperty("error")){
      log.warn('main.mjs:  [ERROR] in external module has occurred:  ' + err.error + '\n' + 
      'Object:  ' + JSON.stringify(err.error) + '\n' + 
      'Stack :   ' + err.error.stack)
    }
  } 
  catch (errCaught) {
    log.warn('main.mjs:  [ERROR]  Exception when handling an error.  This should be logged as a bug on the dayjob GitHub page.  Error:  ' + String(errCaught))
...
function logAndDisplayError(err) {
  ...
  log.warn('main.mjs:  [ERROR] Error reported to the user: (' + notifications[err.message].errorType + ') ' + notifications[err.message].title + ': ' + notifications[err.message].description + externalError);
  }
  catch (errCaught){
...
ipcMain.handle('getVersionInfo', async () => { //TODO: Async and not consistent with other IPC handlers
  try {
      ...
      log.warn('main.mjs: Retrieved app version info: ' + packageJson.version);
...
ipcMain.on('buttonPress', (event, data) => {
  log.warn('main.mjs:  Button press received with ID "' + data + '"');
  switch (data){
      ...
      case "authorise_dayjob":      // Event raised by dynamic 'buttonCta' in notification window
        log.warn('main.mjs:  Event authorise_dayjob received by main process.');
        authoriseDayjob();
        break;
      case "connect_api":           // Event raised by dynamic 'buttonCta' in notification window
        log.warn('main.mjs:  Event connect_api received by main process.');
        connectApi();
        break;
      case "playlist_settings":     // Event raised by dynamic 'buttonCta' in notification window
        log.warn('main.mjs:  Event playlist_settings received by main process.');
        openPrefsWindow();
        break;
  }
});
...
ipcMain.handle('setPref', async (event, pref, value) => {
  ...
  log.warn('main.mjs:  IPC is setting preference \'' + pref + '\' with value: ' + value)
  return prefsLocal.setPref(pref, value);
});

ipcMain.handle('setPlaylists', async (event, value) => {
  // TODO - The implementation in ui-preferences.js means a disk write occurs after every single keystroke - a delay should be imposed to save n seconds after the last change
  log.warn('main.mjs:  IPC has received an updated playlist JSON and is saving it.')
  playlists = value;
  return prefsLocal.setPref('dayjob_playlists_v1', value);
});

ipcMain.handle('logAndDisplayError', async (event, msg) => {
  log.warn('main.mjs:  IPC has received an error from the renderer and it will be reported to the user: ' + msg)
