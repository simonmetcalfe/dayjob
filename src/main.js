...
/**
   * iohook test version
   * -----------------------------------------------------------------
   * Alternative method for capturing keyboard shortcuts to allow
   * dayjob to disinguish between CTRL+ALT and ALTGR. 
   * 
   * Allows use of ALTGR for special characters on Euro keyboards
   * while dayjob can use CTRL+ALT for keyboard shortcuts.
 */


 if (prefsLocal.getPref('dayjob_iohook_keyboard_listener') == undefined){prefsLocal.setPref('dayjob_iohook_keyboard_listener',false)}

 if (!prefsLocal.getPref('dayjob_iohook_keyboard_listener')) {
    log.warn('main.js:  Keyboard listenener:  GlobalShortcut.');
...
if (prefsLocal.getPref('dayjob_iohook_keyboard_listener')) {
  log.warn('main.js:  Keyboard listenener:  ioHook (EXPERIMENTAL)');
  ...
  const ctrlAltMinus = ioHook.registerShortcut([29, 56, 12], (KEYS) =>     {keyPressed({modifiers: ["Control","Alt"],key: "-"})});
  const ctrlAltPlus = ioHook.registerShortcut([29, 56, 13], (KEYS) =>      {keyPressed({modifiers: ["Control","Alt"],key: "="})});
  ...
  const ctrlAltShfMinus = ioHook.registerShortcut([29, 56, 42, 12], (KEYS) =>     {keyPressed({modifiers: ["Control","Alt","Shift"],key: "-"})});
  const ctrlAltShfPlus = ioHook.registerShortcut([29, 56, 42, 13], (KEYS) =>      {keyPressed({modifiers: ["Control","Alt","Shift"],key: "="})});
